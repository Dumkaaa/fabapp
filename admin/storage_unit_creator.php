...
if(filter_input(INPUT_GET, "drawer")) {
	...
	$Drawer = StorageUnit::get_units_for_drawer($drawer_number);  // array of unit objects
}
...
elseif($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_drawer"])) {
	...
	$errors = StorageUnit::delete_drawer($selected_drawer_number);
	...
	//IMPORTANT: know if allowed to DELETE QUERIES
}
...
elseif($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["copy_unit_button"])) {
	...
	if(StorageUnit::copy_drawer($new_drawer_number, $drawer_number)) {
...
elseif($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_unit_button"])) {
	$unit = filter_input(INPUT_POST, "deletion_input");
	...
	if(StorageUnit::drawer_exists($drawer_number)) header("Location:storage_unit_creator.php?drawer=$drawer_number");
...
<div id="page-wrapper">
					...
					<table class='table'>
						<tr>
							...
							<td class='col-md-6'>
								...
								<?php 
									$drawer_designations = StorageUnit::get_unique_drawers();
									$drawer_indicator = "'".implode("','", $drawer_designations)."'";
									foreach($drawer_designations as $drawer_instance) {
							...
							<tr>
								...
								<td>
									<div class="input-group" style='padding:8px;'>
										...
										<input id='new_drawer_row_input' name='new_drawer_row_input' class='form-control' 
								...
								<td>
									<div class="input-group" style='padding:8px;'>
										...
										onkeyup='new_drawer();' type='number' min='1' max='50'/>
									</div>
								</td>
							...
							<tr>
								...
								<td>
									<div class="input-group" style='padding:8px;'>
										...
										<select id='new_drawer_type_input' name='new_drawer_type_input' class='form-control' 
										onkeyup='enable_new_drawer_button();'>
										<?php $options = explode(";", $sv['drawer_types']);
	...
	<!------------------------- EDIT CURRENT DRAWER -------------------------->
	<?php if(isset($Drawer)) {
		if(!$Drawer[0]) {  // if no data for drawer
			$_SESSION['error_msg'] = "Drawer '$drawer_number' Not Found";
			header("Location:./storage_unit_creator.php");
		}
		$unit_indicators_for_drawer = "'".implode("','", StorageUnit::get_drawer_units_labels($drawer_number))."'";
		?>
		<div class="panel panel-default">
			<div class="panel-heading">
				<table width='100%'>
			...
			<div id='drawer_editor' class='collapse in drawer_collapse'>
				...
				<div align='center'>
					<?php echo $Drawer[0]->create_box_selection($Drawer); ?>
					<form method="post">
						<table width='100%'>
							<tr style='padding:8px;'>
								<td style='padding:8px;width:35%;' align='center'>
									<button onclick='set_copying(this);' 
									id='copy_toggle' class='btn btn-default' type='button'>Copy Unit</button>
								</td>
							...
							<tr id='partition_unit_div' style='width:100%;' hidden>
								...
								<td class='col-md-4'>
									<div class="input-group" style='padding:8px;'>
										...
										<select id='partition_unit_type' name='partition_unit_type' class='form-control'
										onchange='enable_partition_button();'>
...
<script>
	...
	*/
	function reset_drawer_appearance() {
		...
		<?php if(isset($Drawer)) { ?>
			...
			// reset deleting
			deleting = false;
			var delete_button = document.getElementById("delete_unit_button");
	...
	*/
	function new_drawer() {
		enable_new_drawer_button("<?php echo $drawer_indicator; ?>");
	...
	function enable_new_drawer_button() {
		...
		var input = document.getElementById("new_drawer_name_input").value;
		var preexisting = [<?php echo $drawer_indicator; ?>];
		prevent_duplicate_creation(button, input, "Create New Drawer", preexisting);

		if(input && document.getElementById("new_drawer_row_input").value 
		&& document.getElementById("new_drawer_column_input").value && document.getElementById("new_drawer_partition").value
		&& document.getElementById("new_drawer_indicator_input").value && button.innerHTML != "Already exists")
	...
	function enable_copy_button() {
		...
		var input = document.getElementById("copy_drawer_input").value;
		var preexisting = [<?php echo $drawer_indicator; ?>];
		prevent_duplicate_creation(button, input, "Copy", preexisting);

		if(input && button.innerHTML != "Already exists") button.disabled = false;
		else button.disabled = true;
	}
	...
	*/
	...
	function enable_partition_button() {
		var input = document.getElementById("partition_label").value;
		var button = document.getElementById("partition_unit");
		var preexisting = [<?php echo $unit_indicators_for_drawer; ?>];
		prevent_duplicate_creation(button, input, "Partition", preexisting);

		// if values good, enable button
		if(input && document.getElementById("partition_unit_type").value 
		&& document.getElementById("edit_partition_input").value && button.innerHTML != "Already exists")
			button.disabled = false;
		else button.disabled = true;
	}
